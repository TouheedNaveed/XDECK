...
export default function InputTab({ sendMessage, onBack }: InputTabProps) {
  return (
    <div
      ...
      style={{ background: '#080d1a', touchAction: 'none', WebkitOverflowScrolling: 'auto' }}
    >
...
function Trackpad({ sendMessage }: { sendMessage: (msg: WSMessage) => boolean }) {
  ...
  const handleTouchStart = useCallback((e: React.TouchEvent) => {
    ...
    if (e.touches.length === 1) {
      sendMouse('click', { button: 1, down: true });
    }
  }, [sendMouse]);
  ...
  return (
    <div
      ...
      style={{ height: '38%', minHeight: 140, background: 'rgba(255,255,255,0.02)' }}
    >
      {/* Grid pattern */}
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <div className={`w-12 h-12 rounded-full border transition-all duration-100 ${
      ...
      <div className="absolute bottom-2 left-0 right-0 text-center pointer-events-none">
        <span className="text-[9px] text-white/15 font-mono">
          Drag = move · Tap = click · 2-finger = scroll · Long press = right-click
        </span>
      </div>
    </div>
  );
    ...
    <div className="flex-1 flex flex-col overflow-hidden min-h-0" style={{ background: 'rgba(255,255,255,0.01)' }}>
      ...
      <div className="flex items-center gap-1.5 px-3 py-1.5 border-t border-white/5 shrink-0">
        ...
        <button
          ...
          className={`text-[10px] px-2 py-1 rounded-lg transition-all ${
            showFKeys ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/30' : 'bg-white/5 text-white/40 border border-white/10'
          }`}
        >
          F-keys
        </button>
      ...
      {textMode ? (
        ...
        <div className="flex-1 overflow-y-auto px-1.5 py-1 min-h-0" style={{ WebkitOverflowScrolling: 'touch' }}>
          {/* Modifier keys */}
            ...
            <div key={ri} className="flex gap-0.5 mb-0.5">
              {row.map(key => (
                <button
                  ...
                  className="flex-1 py-2 rounded-md text-[12px] font-medium bg-white/5 text-white/70 border border-white/8 active:bg-indigo-500/20 active:text-indigo-200 active:border-indigo-500/30 transition-all select-none"
                >
                  {key}
                </button>
              ))}
          ...
          <div className="flex gap-0.5 mb-0.5">
            <button
              ...
              className="w-12 py-2 rounded-md text-[10px] font-medium bg-white/5 text-white/40 border border-white/8 active:bg-indigo-500/20 select-none"
            >
              Tab
            </button>
            <button
              onTouchStart={(e) => { e.preventDefault(); handleKeyPress('⌫'); }}
              className="flex-1 py-2 rounded-md text-[12px] font-medium bg-white/5 text-white/70 border border-white/8 active:bg-red-500/20 active:text-red-300 select-none"
            >
              ⌫
            </button>
            <button
              onTouchStart={(e) => { e.preventDefault(); handleKeyPress(' '); }}
              className="flex-[3] py-2 rounded-md text-[11px] font-medium bg-white/5 text-white/50 border border-white/8 active:bg-indigo-500/20 select-none"
            >
              Space
            </button>
            <button
              onTouchStart={(e) => { e.preventDefault(); handleKeyPress('⏎'); }}
              className="flex-[1.5] py-2 rounded-md text-[12px] font-semibold bg-indigo-500/15 text-indigo-300 border border-indigo-500/20 active:bg-indigo-500/30 select-none"
            >
              Enter
            </button>
          </div>
          ...
          {showFKeys && (
            <div className="flex flex-wrap gap-0.5 mb-0.5">
              {['F1','F2','F3','F4','F5','F6','F7','F8','F9','F10','F11','F12'].map(fk => (
                <button
                  key={fk}
                  onTouchStart={(e) => { e.preventDefault(); handleKeyPress(fk); }}
                  className="flex-1 min-w-[32px] py-1.5 rounded-md text-[9px] bg-white/5 text-white/40 border border-white/8 active:bg-indigo-500/20 select-none"
                >
                  {fk}
                </button>
              ))}
          ...
          <div className="flex gap-0.5 mb-1.5">
            <button
              onTouchStart={(e) => { e.preventDefault(); handleKeyPress('⎋'); }}
              className="flex-1 py-2 rounded-md text-[11px] font-medium bg-white/5 text-white/40 border border-white/8 active:bg-red-500/20 active:text-red-300 select-none"
            >
              Esc
            </button>
            <button
              onTouchStart={(e) => { e.preventDefault(); handleKeyPress('Print'); }}
              className="flex-1 py-2 rounded-md text-[10px] bg-white/5 text-white/30 border border-white/8 active:bg-indigo-500/20 select-none"
            >
              PrtSc
            </button>
            <button
              onTouchStart={(e) => { e.preventDefault(); handleKeyPress('Delete'); }}
              className="flex-1 py-2 rounded-md text-[10px] bg-white/5 text-white/30 border border-white/8 active:bg-red-500/20 active:text-red-300 select-none"
            >
              Del
            </button>
          </div>
